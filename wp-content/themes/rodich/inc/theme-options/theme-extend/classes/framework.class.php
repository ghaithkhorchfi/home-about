...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'rodich' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'rodich' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'rodich' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo ( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'rodich' ) .'</span>' : '';

      submit_button( esc_html__( 'Save', 'rodich' ), 'primary cs-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'rodich' ) ) );
      submit_button( esc_html__( 'Restore', 'rodich' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( esc_html__( 'Reset All Options', 'rodich' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'rodich' ) .'</a>' : '';
        ...
        echo '<div class="cs-content">';
          ...
          foreach( $this->sections as $section ) {
            ...
            if( isset( $section['fields'] ) ) {
              ...
              // Custom Changes - video_link (a) tag.
              if ( isset( $section['video_link'] ) ) {
                echo '<a href="'. $section['video_link'] .'" target="_blank" class="cs-section-video"><i class="fa fa-youtube-play"></i> <span>'. esc_html__('How to?', 'rodich') .'</span></a>';
              }
              echo '</div>';
