<?php
/*
 * All Back-End Helper Functions for Rodich Theme
 * Author & Copyright: VictorThemes
 * URL: http://themeforest.net/user/VictorThemes
 */

/* Validate px entered in field */
if( ! function_exists( 'rodich_check_px' ) ) {
  function rodich_check_px( $num ) {
    return ( is_numeric( $num ) ) ? $num . 'px' : $num;
  }
}

/* Escape Strings */
if( ! function_exists( 'rodich_vt_esc_string' ) ) {
  function rodich_vt_esc_string( $num ) {
    return preg_replace('/\D/', '', $num);
  }
}

/* Escape Numbers */
if( ! function_exists( 'rodich_vt_esc_number' ) ) {
  function rodich_vt_esc_number( $num ) {
    return preg_replace('/[^a-zA-Z]/', '', $num);
  }
}

/* Compress CSS */
if ( ! function_exists( 'rodich_compress_css_lines' ) ) {
  function rodich_compress_css_lines( $css ) {
...
if( ! function_exists( 'rodich_vt_upload_mimes' ) ) {
  function rodich_vt_upload_mimes( $mimes ) {
  ...
  add_filter( 'upload_mimes', 'rodich_vt_upload_mimes' );
}

/* Custom WordPress admin login logo */
if( ! function_exists( 'rodich_theme_login_logo' ) ) {
  function rodich_theme_login_logo() {
    $login_logo = cs_get_option('brand_logo_wp');
    if($login_logo) {
      $login_logo_url = wp_get_attachment_url($login_logo);
    } else {
      $login_logo_url = RODICH_IMAGES . '/logo.png';
    }
    if($login_logo) {
    echo "
      <style>
  ...
  add_action('login_head', 'rodich_theme_login_logo');
}

/* WordPress admin login logo link */
if( ! function_exists( 'rodich_login_url' ) ) {
  function rodich_login_url() {
    return site_url();
  }
  add_filter( 'login_headerurl', 'rodich_login_url', 10, 4 );
}

/* WordPress admin login logo link */
if( ! function_exists( 'rodich_login_title' ) ) {
  function rodich_login_title() {
    return get_bloginfo('name');
  }
  add_filter('login_headertext', 'rodich_login_title');
}
...
if ( ! function_exists( 'rodich_tinymce_btns_font' ) ) {
  function rodich_tinymce_btns_font( $buttons ) {
    array_unshift( $buttons, 'fontselect' ); // Add Font Select
    array_unshift( $buttons, 'fontsizeselect' ); // Add Font Size Select
    return $buttons;
  }
  add_filter( 'mce_buttons_2', 'rodich_tinymce_btns_font' );
}

// Customize mce editor font sizes
if ( ! function_exists( 'rodich_tinymce_sizes' ) ) {
  function rodich_tinymce_sizes( $initArray ){
    $initArray['fontsize_formats'] = "9px 10px 12px 13px 14px 16px 18px 21px 24px 28px 32px 36px";
    return $initArray;
  }
  add_filter( 'tiny_mce_before_init', 'rodich_tinymce_sizes' );
}

// Customize mce editor font family
if ( ! function_exists( 'rodich_tinmymce_family' ) ) {
  function rodich_tinmymce_family( $initArray ) {
  ...
  add_filter( 'tiny_mce_before_init', 'rodich_tinmymce_family' );
}

/* HEX to RGB */
if( ! function_exists( 'rodich_vt_hex2rgb' ) ) {
  function rodich_vt_hex2rgb( $hexcolor, $opacity = 1 ) {
...
if( ! function_exists( 'rodich_vt_yoast_metabox' ) ) {
  function rodich_vt_yoast_metabox() {
    return 'low';
  }
  add_filter( 'wpseo_metabox_prio', 'rodich_vt_yoast_metabox' );
}
...
if ( is_admin() ) {
  ...
  if( ! function_exists( 'rodich_remove_rev_slider_meta_boxes' ) ) {
    function rodich_remove_rev_slider_meta_boxes() {
    ...
    add_action( 'do_meta_boxes', 'rodich_remove_rev_slider_meta_boxes' );
  }
