...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract{
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary cs-shortcode" data-editor-id="'. $editor_id .'">'. esc_html__( 'Add Shortcode', 'rodich' ) .'</a>';
    }
  ...
  ?>
    <div id="cs-shortcode-dialog" class="cs-dialog" title="<?php esc_html_e( 'Add Shortcode', 'rodich' ); ?>">
      <div class="cs-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>cs-dialog-select" data-placeholder="<?php esc_html_e( 'Select a shortcode', 'rodich' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="cs-insert-button hidden">
        <a href="#" class="button button-primary cs-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'rodich' ); ?></a>
      </div>
    </div>
  <?php
  }
