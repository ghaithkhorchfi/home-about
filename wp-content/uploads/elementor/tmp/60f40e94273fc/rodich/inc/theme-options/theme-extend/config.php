 ...
 */
require RODICH_ADMIN_PATH .'/cs-framework.php';
require RODICH_FRAMEWORK .'/theme-options/theme-options.php';
require RODICH_FRAMEWORK .'/theme-options/theme-customizer.php';
require RODICH_FRAMEWORK .'/theme-options/theme-metabox.php';
 ...
 */
function rodich_vt_cs_framework_override() {
  return 'inc/theme-options/theme-extend';
}
add_filter( 'cs_framework_override', 'rodich_vt_cs_framework_override' );

/**
 * Custom New Font Family
 */
if( ! function_exists( 'rodich_vt_custom_font_upload' ) ) {
  function rodich_vt_custom_font_upload( $db_value ) {
  ...
  add_action( 'cs_typography_family', 'rodich_vt_custom_font_upload' );
}

/**
 * Check Custom Font
 */
if ( ! function_exists( 'rodich_vt_custom_upload_font' ) ) {
  function rodich_vt_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'rodich_vt_registered_sidebars' ) ) {
  function rodich_vt_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'rodich_vt_google_fonts' ) ) {
  function rodich_vt_google_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'rodich-google-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'rodich_vt_get_typography' ) ) {
  function rodich_vt_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? rodich_vt_esc_string( $font['font']['variant'] ) : 400;
          $style   = rodich_vt_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. rodich_check_px( $font['size'] ) .';' : '';
