    ...
    if( ! empty( $jsons ) ) {
      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          echo '<h4 class="cs-icon-title">'. esc_html__( 'Error! Can not load json file.', 'rodich' ) .'</h4>';
        }
...
if( ! function_exists( 'cs_set_icons' ) ) {
  function cs_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. esc_html__( 'Add Icon', 'rodich' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. esc_html__( 'Search a Icon...', 'rodich' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. esc_html__( 'Loading...', 'rodich' ) .'</div></div>';
    echo '</div>';
